...
static void setup_hardcoded_group_table(oc_group_table_t *entry, uint32_t ga,
                                        uint32_t grpid) {
  if (entry == NULL) {
    return;
  }
...
void knx_apply_presets(const knx_preset_t *preset) {
  bool pm = false;
  ...
  if (setup_hardcoded_got(preset->ga, preset->got_href,
                          (oc_cflag_mask_t)preset->got_cflags) < 0) {
    LOG_ERR("failed to configure group object table");
    return;
  }

  if (preset->is_publisher) {
    setup_hardcoded_group_table(oc_core_get_publisher_table_entry(0),
                                preset->ga, preset->grpid);
  } else {
    setup_hardcoded_group_table(oc_core_get_recipient_table_entry(0),
                                preset->ga, preset->grpid);
  }

  if (setup_hardcoded_access_token(preset) < 0) {
    LOG_ERR("failed to configure group OSCORE access token");
    return;
  }
