...
static struct {
   ...
   * wake the poll loop early; the condvar carries that wake-up.
   */
  struct k_mutex event_mutex;
  struct k_condvar event_condvar;
...
void signal_event_loop(void) {
  k_mutex_lock(&knx_ctx.event_mutex, K_FOREVER);
  k_condvar_signal(&knx_ctx.event_condvar);
  k_mutex_unlock(&knx_ctx.event_mutex);
}
...
static void knx_thread_entry(void *p1, void *p2, void *p3) {
  ...
  while (atomic_get(&knx_ctx.running)) {
    ...
    k_mutex_lock(&knx_ctx.event_mutex, K_FOREVER);
    /* Work that landed after the drain above but before we slept: handle it
     * now instead of waiting.
     */
    if (knx_ctx.work_handler != NULL && atomic_get(&knx_ctx.work_pending)) {
      k_mutex_unlock(&knx_ctx.event_mutex);
      continue;
    }
    if (next_event == 0) {
      k_condvar_wait(&knx_ctx.event_condvar, &knx_ctx.event_mutex, K_FOREVER);
    } else {
      ...
      k_condvar_wait(&knx_ctx.event_condvar, &knx_ctx.event_mutex, timeout);
    }
    k_mutex_unlock(&knx_ctx.event_mutex);
  }
...
int knx_app_start(void) {
  ...
  k_mutex_init(&knx_ctx.event_mutex);
  k_condvar_init(&knx_ctx.event_condvar);

  int ret = app_initialize_stack(NULL);

  if (ret < 0) {
    LOG_ERR("app_initialize_stack failed: %d", ret);
    return ret;
  }
