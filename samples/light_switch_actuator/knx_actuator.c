...
/* Application-specific output: the switched load is shown on LED2. */
#define APP_LED DK_LED2

static void actuator_set_light(bool on) { dk_set_led(APP_LED, on); }
...
void app_register_device(void) { knx_device_register(&actuator_device); }
