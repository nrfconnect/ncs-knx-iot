 ...
 */
#define KNX_DP_ID(channel, point)                                              \
  ((uint16_t)(((channel) << 8) | ((point) & 0xFF)))
...
/* The light swich only requires boolean data, prepared the code for easy
 * extending it to other types in the future*/
typedef enum {
  KNX_DPT_BOOL = 0,
} knx_dpt_kind_t;
...
typedef struct {
  ...
  int mirror_to; /* datapoint id mirrored + announced on write, or KNX_DP_NONE
                  */
...
typedef struct {
  ...
  knx_lifecycle_cb_t
      on_init; /* after stack init + presets, before the KNX thread starts */
